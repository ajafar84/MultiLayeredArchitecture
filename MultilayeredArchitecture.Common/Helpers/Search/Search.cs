using MultilayeredArchitecture.Common.Classes.Search;
using System;
...
namespace MultilayeredArchitecture.Common.Helpers.Search
{
    public static class Search
    {
        ...
        public static IQueryable<T> DynamicSearch<T>(this IQueryable<T> source, SearchModel searchModel)
        {
            ...
            if (searchModel.SearchFields != null)
            {
                ...
                {
                    if (!tProperties.Exists(p => p.Name == field.Name))
                    {
                        continue;
                    }
                    ...
                    {
                        Type actualType = tProperties.Where(p => p.Name == field.Name).First().PropertyType;
                        ...
                        switch (field.Operator)
                        {
                            case "Contain":
                                try
                                {
                                    ...
                                    {
                                        source = source.Where($"{field.Name}.Value.ToString().Trim().Contains(@0)", field.Value.Trim());
                                    }
                                    else
                                    {
                                        source = source.Where($"{field.Name}.ToString().Trim().Contains(@0)", field.Value.Trim());
                                    }
                                }
                            ...
                            default:
                                try
                                {
                                    ...
                                    source = source.Where($"{field.Name} {Opertors[field.Operator]} @0", safeValue);
                                }
                                catch (Exception) { continue; }
                                break;
                        }
